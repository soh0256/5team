...
const Register = (props) => {
  ...
  const handleAddress = () => {
    let win = window.open("http://localhost:3000/address", "blank", "width=800, 600");
  }

  const importantPopup = () => {
    window.open("http://localhost:3000/ipopup", "새창", "width=800, height=600, resizeable=yes");
  }

  const marketingPopup = () => {
    window.open("http://localhost:3000/mpopup", "새창", "width=800, height=600, resizeable=yes");
  }
