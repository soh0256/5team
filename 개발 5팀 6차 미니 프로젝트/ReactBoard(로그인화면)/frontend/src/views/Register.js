...
const Register = (props) => {
  ...
  useEffect(() => {
    if (location.state) {
      // 회원수정페인지 확인하는 if문
      fetch(
        `http://192.168.0.6:3001/user/update/${location.state.idx}`,
        {
          method: "GET",
        }
      )
  ...
  const registerSubmit = async (event) => {
    ...
    //   `http://192.168.0.6:3000/user/register`,
    //   reqFormData,
    //   config
    // );
  ...
  //   post(
  //     `http://192.168.0.6:3010/api/user/update/${location.state.idx}`,
  //     reqFormData,
  //     config
  //   ).then((res) => {
  ...
  const handleAddress = () => {
    let win = window.open("http://192.168.0.6:3010/address", "blank", "width=800, 600");
  }

  const importantPopup = () => {
    window.open("http://192.168.0.6:3010/ipopup", "새창", "width=800, height=600, resizeable=yes");
  }

  const marketingPopup = () => {
    window.open("http://192.168.0.6:3010/mpopup", "새창", "width=800, height=600, resizeable=yes");
  }
