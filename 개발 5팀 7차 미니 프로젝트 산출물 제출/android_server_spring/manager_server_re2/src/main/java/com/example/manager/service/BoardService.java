...
public class BoardService {
     ...
     * 제목으로 조회
     */
    public Board findTitle(Board title, Board content) {
        Optional<Board> board = boardRepository.findByTitle(title.getTitle(), content.getContent());
        if (board.isPresent()) {
            return board.get();
        } else {
            throw new MemberNotFoundException("There is no member by " + title.getId());
        }
    }
