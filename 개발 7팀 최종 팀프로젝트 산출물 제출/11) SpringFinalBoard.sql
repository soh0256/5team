...
-- 게시판 좋아요/싫어요
DROP TABLE BOARD_CNT CASCADE CONSTRAINT;
CREATE TABLE BOARD_CNT(
  BOARD_CNT_NO VARCHAR2(30),		-- 번호
  ID VARCHAR2(20),            		-- 유저코드
  BOARD_NO VARCHAR2(30),             	-- 게시글번호
  BOARD_GCNT NUMBER(3) DEFAULT 0,	-- 좋아요
  BOARD_BCNT NUMBER(3) DEFAULT 0,	-- 싫어요
  CONSTRAINT TB_BOARD_CNT_PK PRIMARY KEY(BOARD_CNT_NO),
  CONSTRAINT TB_BOARD_CNT_FK FOREIGN KEY(BOARD_NO) REFERENCES TB_BOARD(BOARD_NO) ON DELETE CASCADE
);
